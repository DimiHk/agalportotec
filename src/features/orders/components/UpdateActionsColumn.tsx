...
export const UpdateActionsColumn = ({ orderPart }: any) => {
  const { reference, name, price, id } = orderPart.data;

  const { rowIndex } = orderPart;

  const { isOpen, onOpen, onClose } = useDisclosure();

  const { handleRemoveOrderPart, handleUpdatePart } = useUpdateOrderDetails();
      ...
      <Modal size={"2xl"} isOpen={isOpen} onClose={onClose}>
        ...
        <form
          onSubmit={handleSubmit(({ name, reference, price }) =>
            handleUpdatePart(
              { name, reference, price, id: orderPart.data.id ?? undefined },
              rowIndex
            )
          )}
        >
