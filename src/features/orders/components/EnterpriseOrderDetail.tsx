...
export const EnterpriseDetailOrder = () => {
  ...
  const { orderDetails, mutate } =
    ordersService.handleGetEnterpriseClientOrderDetails(
      clientId as string,
      orderId as string
    );
...
const MainContainer = ({
  ...
  currentTab?: string;
  orderDetails?: EnterpriseOrderDetailsResponse;
  mutate: KeyedMutator<EnterpriseOrderDetailsResponse>;
}) => {
  const router = useRouter();
  ...
  const handleUpdateEnterpriseDetailsOrder = (data?: any) => {
    ordersService
  ...
  return (
    <React.Fragment>
      {currentTab === "client" &&
        ...
        paymentMethod && (
          <React.Fragment>
            <form onSubmit={handleSubmit(handleUpdateEnterpriseDetailsOrder)}>
              <FormControl
                ...
                <Flex width={"full"} direction={"column"} padding={4} gap={4}>
                  ...
                  <Flex justify={"center"} align={"center"} gap={4}>
                    ...
                    <FormControl>
                      ...
                      <Input
                        borderColor={"gray.400"}
                        defaultValue={orderDetails?.client.employeeName}
                        disabled
                        cursor={"text!important"}
                      />
                    </FormControl>
                    ...
                    <FormControl>
                      ...
                      <Input
                        defaultValue={formatString(
                          orderDetails?.client.generalPhoneNumber
                        )}
                        borderColor={"gray.400"}
                        disabled
                        cursor={"text!important"}
      ...
      {currentTab === "orders" && (
        <React.Fragment>
          ...
          >
            <Flex justify={"flex-start"} width={"full"} gap={2}>
              <Button
                onClick={() => handleUpdateEnterpriseDetailsOrder()}
                size={"sm"}
                rounded={"full"}
                shadow={"base"}
                backgroundColor={"green.300"}
