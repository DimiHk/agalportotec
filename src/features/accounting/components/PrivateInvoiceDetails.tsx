...
import { PrivateInvoiceResponse, UpdatePrivateInvoiceRequest } from "@/models";
import { updatePrivateInvoiceSchema } from "@/schemas";
...
export const PrivateInvoiceDetails = () => {
  ...
  const { invoice } = clientService.handleGetPrivateInvoiceById(
    clientId as string,
    invoiceId as string
  );
  ...
  useEffect(() => {
    if (invoice) {
      handleSetDefaultValues(invoice);
    }
  }, [invoice]);
...
const MainContainer = ({
  ...
  onClose: () => void;
  currentTab: string;
  invoice: PrivateInvoiceResponse;
}) => {
...
const UserSettings = () => {
  ...
  const { client } = clientService.handleGetPrivateClientDetails(
    clientId as string
  );
...
const InvoiceSettings = () => {
  ...
  const { mutate } = clientService.handleGetPrivateInvoiceById(
    clientId as string,
    invoiceId as string
  );
  ...
  const { register, handleSubmit, setValue, watch } =
    useSubmitForm<UpdatePrivateInvoiceRequest>(updatePrivateInvoiceSchema);
